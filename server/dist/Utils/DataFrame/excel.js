...
const fs = require("fs");
class ConvertExcel {
    async orderListToExcel(rows, fileName, folderPath, filesPath) {
    ...
    async memberListToExcel(rows, fileName, folderPath, filesPath) {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet("訂單資料");
        worksheet.columns = [
            { header: "會員卡號", key: "cardId", width: 15 },
    ...
    async couponDetailListToExcel(rows, fileName, couponTypeStr, folderPath, filesPath) {
        const workbook = new ExcelJS.Workbook();
    ...
    async rewardDetailToExcel(rows, fileName, folderPath, filesPath) {
        const workbook = new ExcelJS.Workbook();
    ...
    async motSendLogToExcel(rows, fileName, folderPath, filesPath) {
        const workbook = new ExcelJS.Workbook();
